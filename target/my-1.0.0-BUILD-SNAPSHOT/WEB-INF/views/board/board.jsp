			...
			<div class="row">
				...
				<div class="category-bar">
					<ul>
						...
						<li><a href="/board/2">관리비</a></li>
						<li><a href="/board/3">절약 팁</a></li>
						<li><a href="/board/4">유머</a></li>
						<li><a href="/board/5">우리 아파트 장터</a></li>
						<li><a href="/board/6">주변 상권 이야기</a></li>
						<li><a href="/board/7">부동산 이야기</a></li>
					</ul>
				</div>
				...
				<div class="page-submit-wrapper">
				    <div class="pagination">
				        ...
				        <c:forEach var="pageNum" begin="${boardData.startPage}" end="${boardData.endPage}">
					        <c:choose>
					        	<c:when test="${boardMasterId == null}">
					        		<a href="/board?page=${pageNum}&size=${boardData.pageSize}" class="${pageNum eq boardData.curPage ? 'current-page' : ''}">${pageNum}</a>
					        	</c:when>
					        	<c:otherwise>
					        		<a href="/board/${boardMasterId}?page=${pageNum}&size=${boardData.pageSize}" class="${pageNum eq boardData.curPage ? 'current-page' : ''}">${pageNum}</a>
					        	</c:otherwise>
					        </c:choose>
				        </c:forEach>
